...
もう十分考えるべき問題は洗い出せたと思ったそのとき、新たな問題が現れます。共有リソースをロックすることは[readers-writers problem](http://en.wikipedia.org/wiki/Readers-writers_problem)に行き当たることがあります。多くの場合、読み込みのアクセスを同時に１つだけに制限することは無駄になります。そのため、読み込み時のロックは、そのリソースに対して書き込みロックがひとつもない場合のみ許可されています。この状況では、書き込みロックの獲得を待っているスレッドは、その間に発生するより多くの読み込みロックによってstarvedにされる可能性があります。

この問題を解決するにはシンプルな読み込み／書き込みロックではなく、[書き込みに優先権を与えたり](http://en.wikipedia.org/wiki/Readers–writer_lock)、[read-copy-upadte](http://ja.wikipedia.org/wiki/リード・コピー・アップデート)アルゴリズムを使うといった、より高度な解決策が必要です。Danielが[low-level concurrency tequniques](http://www.objc.io/issue-2/low-level-concurrency-apis.html#multiple-readers-single-writer)の中で、複数のリーダー／ひとつのライターのパターンでGCDを使った実装方法について書いています。
...
![優先順位逆転](http://www.objc.io/images/issue-2/priority-inversion@2x.png)
...
このことから導かれる教訓は、優先度の異なる複数のキューを使うことは理論上よいものに見えますが、実際は並列プログラムにさらなる複雑性と予測不能性を加えてしまうということです。そしてもしあなたが、優先度の高いタスクが原因もわからず止まってしまう奇妙な問題に直面したら、多分この記事と優先順位逆転という問題のことを思い出すでしょう。NASAのエンジニアさえ直面したこの問題を。

### まとめ

どんなにAPIがわかりやすいものに見えても、並列プログラミングとそれに関する問題がいかに複雑かが伝わったでしょうか。結果としての振る舞いはすぐに見渡すのが難しくなり、この種の問題をデバッグすることは非常に難しくなります。

一方で、並列プログラムはモダンなマルチコアCPUのパワーを活用するための強力なツールでもあります。いかに並列モデルをシンプルに保ち、ロックが必要になる箇所を減らすかが鍵になります。

おすすめの安全なパターンは次の通りです。まずメインスレッドで必要なデータを引っ張って、実際の処理はoperation queueを使ってバックグラウンドで行い、最後にメインキューに戻ってバックグラウンド処理の結果を届けます。こうすることでプログラマー自身がロックをする必要がなくなり、間違いが起こる可能性を大幅に減らすことができます。
